...
interface RidesPerWeekBySurfaceData {
  week: string;
  weekLabel: string;
...
interface RidesPerWeekChartProps {
  data: RidesPerWeekBySurfaceData[];
}
...
export function RidesPerWeekChart({ data }: RidesPerWeekChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    data.forEach((item) => {
      ...
      weekData[item.surface] = item.count;
    });
  ...
  return (
    <ChartContainer config={chartConfig} className="h-[300px] w-full">
      <BarChart data={chartData} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
        ...
        <Bar dataKey="offroad" fill="var(--color-offroad)" stackId="rides" radius={[0, 0, 0, 0]} />
        <Bar dataKey="virtual" fill="var(--color-virtual)" stackId="rides" radius={[4, 4, 0, 0]} />
      </BarChart>
    </ChartContainer>
  );
}
